...
%% Substrate Level Definition Network
%% There's attempt to keep it in alphabetical order by from node. But this is done manually so results may vary :).
%% The substrate definitions are prior to the graph and the graph emerges from those definitions and their known contexts.
%% Specific morallity, named or higher order theology, or other system are not intended.  
%% Working to examine mechanics, margins and magesty allowig mystery except as a mask for mitigable muddle.
